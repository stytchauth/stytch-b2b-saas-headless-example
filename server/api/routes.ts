...
import { authenticate, authenticateAndAuthorize, getAuthenticatedUserInfo, loadStytch } from '../auth/index.js';
import {
	addIdea,
	deleteIdea,
	getIdeas,
 ...
 */
api.get('/ideas', authenticateAndAuthorize('idea', 'read'), async (req, res) => {
...
api.post('/idea', authenticateAndAuthorize('idea', 'create'), async (req, res) => {
	const { member } = await getAuthenticatedUserInfo({ req })
 ...
 */
api.delete('/idea', authenticateAndAuthorize('idea', 'delete'), async (req, res) => {
...
api.post('/team-settings', authenticateAndAuthorize("stytch.organization", '*'), async (req, res) => {
	const stytch = loadStytch();
	const { member } = await getAuthenticatedUserInfo({ req })
